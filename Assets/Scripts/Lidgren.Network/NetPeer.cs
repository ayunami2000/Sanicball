...
namespace Lidgren.Network
{
	...
	public partial class NetPeer
	{
		...
		private readonly Dictionary<string, NetConnection> m_connectionLookup;
		...
		public NetPeer(NetPeerConfiguration config)
		{
			...
			m_unsentUnconnectedMessages = new NetQueue<NetTuple<string, NetOutgoingMessage>>(2);
			m_connections = new List<NetConnection>();
			m_connectionLookup = new Dictionary<string, NetConnection>();
			m_handshakes = new Dictionary<string, NetConnection>();
            m_senderRemote = ""; // idk
		...
		/// </summary>
		public NetConnection GetConnection(string ep)
		{
		...
		internal void SendLibrary(NetOutgoingMessage msg, string recipient)
		{
			VerifyNetworkThread();
		...
		static string GetNetEndPoint(string host, int port)
		{
			return host + ':' + port;
		}
		...
		/// Create a connection to a remote endpoint
		/// </summary>
		public NetConnection Connect(string remoteEndPoint)
		{
		...
		/// Create a connection to a remote endpoint
		/// </summary>
		public virtual NetConnection Connect(string remoteEndPoint, NetOutgoingMessage hailMessage)
		{
		...
		/// </summary>
		public void RawSend(byte[] arr, int offset, int length, string destination)
		{
