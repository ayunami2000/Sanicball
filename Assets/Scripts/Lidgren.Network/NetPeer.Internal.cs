...
namespace Lidgren.Network
{
	public partial class NetPeer
	{
		...
		internal readonly NetQueue<NetTuple<string, NetOutgoingMessage>> m_unsentUnconnectedMessages;

		internal Dictionary<string, NetConnection> m_handshakes;
		...
		private void BindSocket(bool reBind)
		{
			...
			using (var mutex = new Mutex(false, "Global\\lidgrenSocketBind"))
			{
				try
				{
                    ...
                    var ep = m_configuration.LocalAddress + ':' + (reBind ? m_listenPort : m_configuration.Port);
					m_socket.Bind(ep);
			...
			var boundEp = m_socket.LocalEndPoint as string;
		...
		private void Heartbeat()
		{
			...
			if (delta > (1.0 / (double)maxCHBpS) || delta < 0.0) // max connection heartbeats/second max
			{
				...
				NetTuple<string, NetOutgoingMessage> unsent;
        ...
        private void ReceiveSocketData(double now)
        {
			...
			var ipsender = (string)m_senderRemote;
		...
		internal void HandleIncomingDiscoveryRequest(double now, string senderEndPoint, int ptr, int payloadByteLength)
		{
		...
		internal void HandleIncomingDiscoveryResponse(double now, string senderEndPoint, int ptr, int payloadByteLength)
		{
		...
		private void ReceivedUnconnectedLibraryMessage(double now, string senderEndPoint, NetMessageType tp, int ptr, int payloadByteLength)
		{
			NetConnection shake;
