...
namespace Lidgren.Network
{
	public partial class NetPeer
	{
		...
		public void DiscoverLocalPeers(int serverPort)
		{
			...
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(new NetEndPoint(NetUtility.GetBroadcastAddress(), serverPort), um));
		}
		...
		public bool DiscoverKnownPeer(string host, int serverPort)
		{
			...
			if (address == null)
				return false;
			DiscoverKnownPeer(address + ':' + serverPort);
			return true;
		}
		...
		/// Emit a discovery signal to a single known host
		/// </summary>
		public void DiscoverKnownPeer(string endPoint)
		{
			NetOutgoingMessage om = CreateMessage(0);
			om.m_messageType = NetMessageType.Discovery;
			om.m_recyclingCount = 1;
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(endPoint, om));
		}

		/// <summary>
		/// Send a discovery response message
		/// </summary>
		public void SendDiscoveryResponse(NetOutgoingMessage msg, string recipient)
		{
			if (recipient == null)
			...
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(recipient, msg));
		}
	}
}
