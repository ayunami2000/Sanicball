...
namespace Lidgren.Network
{
	public partial class NetPeer {
			...
			string token)
		{
			...
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(clientExternal, um));
			...
			um.Write(token);
			Interlocked.Increment(ref um.m_recyclingCount);
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(hostExternal, um));
		}
		...
		internal void HandleNatIntroduction(int ptr)
		{
			...
			string remoteInternal = tmp.ReadIPEndPoint();
			string remoteExternal = tmp.ReadIPEndPoint();
			...
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(remoteInternal, punch));
			...
			punch.Write(token);
			Interlocked.Increment(ref punch.m_recyclingCount);
			m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(remoteExternal, punch));
		...
		/// </summary>
		private void HandleNatPunch(int ptr, string senderEndPoint)
		{
			NetIncomingMessage tmp = SetupReadHelperMessage(ptr, 1000); // never mind length
			...
			if (isFromClient)
			{
				...
				m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(senderEndPoint, confirmResponse));
			}
			else
			{
				...
				m_unsentUnconnectedMessages.Enqueue(new NetTuple<string, NetOutgoingMessage>(senderEndPoint, confirmRequest));
			}
		}

		private void HandleNatPunchConfirmRequest(int ptr, string senderEndPoint)
		{
			NetIncomingMessage tmp = SetupReadHelperMessage(ptr, 1000); // never mind length
			var isFromClient = tmp.ReadByte() == ClientByte;
			string token = tmp.ReadString();
		...
		private void HandleNatPunchConfirmed(int ptr, string senderEndPoint)
		{
			NetIncomingMessage tmp = SetupReadHelperMessage(ptr, 1000); // never mind length
			var isFromClient = tmp.ReadByte() == ClientByte;
			if (isFromClient)
